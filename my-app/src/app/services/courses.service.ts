...
export class CoursesService {
  ...
  getItemByID(id: number): Course {
    return this.getList().find((course) => course.id === id) as Course;
  }
  ...
  updateCourse(
    ...
    let itemToUpdate: Course = this.getItemByID(idI);
    const index = courses.indexOf(itemToUpdate);
    console.log(itemToUpdate);
    const updated: Course = {
      id: idI,
      title: titleI ?? itemToUpdate.title,
      creationDate: creationDateI ?? itemToUpdate.creationDate,
      duration: durationI ?? itemToUpdate.duration,
      description: descriptionI ?? itemToUpdate.description,
      topRated: topRatedI ?? itemToUpdate.topRated,
    };
    console.log(updated);
  ...
  deleteItem(id: number) {
    let courses = this.getList();
    let itemToDelete = this.getItemByID(id);
    const index = courses.indexOf(itemToDelete);
